...
export function LocationsEdit({ item }) {
  ...
  async function updateLocation() {
    ...
    if (
      !(formData.typeId && formData.subTypeId && formData.cityId && formData.districtId && formData.source?.length > 0)
    ) {
    ...
    let filteredFormData = Object.entries(formData).filter(([key, value]) => fields.includes(key));

    const updateLocationResponse = await put(`/location/${item.id}`, Object.fromEntries(filteredFormData));
    ...
    <>
      ...
      <Drawer open={isOpen} onClose={toggleDrawer} direction="right" className="!w-[80vw] md:!w-[50vw]" duration={200}>
        ...
        <form onSubmit={handleSubmit} className={'px-8'}>
          ...
          {response?.ok && data?.ok && <div className={'mt-4 text-lg'}>Güncelleme başarılı</div>}
        </form>
      </Drawer>
    </>
  );
