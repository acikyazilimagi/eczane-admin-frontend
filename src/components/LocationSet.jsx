...
export function LocationSet({
  header,
  isOpen,
  onClose,
  item,
  onSubmit = (formData) => formData,
  getDataAfterInputChange = (formData, name, value) => formData,
}) {
  ...
  const handleInputChange = (event) => {
    ...
    var newFormData = getDataAfterInputChange(formData, name, value);
    setFormData(newFormData);
  };

  const handleSubmit = (event) => {
    event.preventDefault();
    const postSubmitFormData = onSubmit(formData);
    setFormData(postSubmitFormData);
  };
    ...
    <>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className={"px-8"}>
            ...
            >
              ...
              {cityData.map((item) => (
                <option
                  value={item.id}
                  key={item.id}
                  selected={formData.cityId === item.id}
                >
                  {item.key}
                </option>
              ))}
              ...
              cityData
                ...
                ?.districts.map((item) => (
                  <option
                    value={item.id}
                    key={item.id}
                    selected={formData.districtId === item.id}
                  >
                    {item.key}
                  </option>
                ))}
            ...
            <Select name={"typeId"} onChange={handleInputChange}>
              ...
              {typeOptions.map((item) => (
                <option
                  value={item.id}
                  key={item.id}
                  selected={formData.typeId === item.id}
                >
                  {item.name}
                </option>
              ))}
          ...
          {response?.ok && data?.ok && (
            <div className={"mt-4 text-lg"}>Kayıt başarılı</div>
          )}
        </form>
      </Drawer>
    </>
